...
const PMApprovedPage = () => {
  ...
  useEffect(() => {
    const fetch = async () => {
      try {
        ...
        setProviders(all.filter((p: Provider) => p.status === "approved"));
      } catch {
  ...
  return (
    <div className="space-y-5">
      <h2 className="text-xl font-bold text-foreground font-display">Approved Providers</h2>
      ...
      <div className="rounded-xl bg-card border border-border overflow-hidden premium-shadow">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-border">
            {providers.length === 0 ? (
              <tr><td colSpan={4} className="px-5 py-12 text-center text-muted-foreground text-xs">No approved providers</td></tr>
            ) : providers.map(p => (
              <tr key={p._id} className="hover:bg-secondary/30 transition-colors">
                ...
                <td className="px-5 py-3">
                  <div className="flex items-center gap-1">
                    <Icon icon="solar:star-bold" className="h-3.5 w-3.5 text-accent" />
                    <span className="text-xs font-semibold text-foreground">{p.rating || "N/A"}</span>
                  </div>
                </td>
              </tr>
            ))}
