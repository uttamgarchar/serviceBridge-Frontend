...
const UserWalletPage = () => {
  ...
  return (
    <div className="space-y-5">
      <h2 className="text-xl font-bold text-foreground font-display">Wallet</h2>
      ...
      <div className="rounded-xl bg-card border border-border p-6 premium-shadow max-w-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-2xl font-extrabold text-foreground font-display">₹{balance}</p>
          </div>
        </div>
      </div>
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-border">
            {transactions.length === 0 ? (
              <tr><td colSpan={4} className="px-5 py-12 text-center text-muted-foreground text-xs">No transactions yet</td></tr>
            ) : transactions.map(t => (
              <tr key={t._id} className="hover:bg-secondary/30 transition-colors">
                <td className="px-5 py-3">
                  <span className={`inline-flex items-center rounded-md px-2 py-0.5 text-[10px] font-bold capitalize ${
                    t.type === "credit" || t.type === "refund" ? "bg-primary/10 text-primary" : "bg-accent/10 text-accent"
                  }`}>{t.type || "—"}</span>
                </td>
