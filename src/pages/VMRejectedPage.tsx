  ...
  useEffect(() => {
    const fetch = async () => {
      try {
        ...
        // ✅ FIXED FILTER
        setProviders(all.filter((p: Provider) => p.verificationStatus === "rejected"));
      } catch {
            ...
            {providers.length === 0 ? (
              ...
              providers.map((p) => (
                <tr key={p._id} className="hover:bg-secondary/30 transition-colors">
                  ...
                  {/* Reason */}
                  <td className="px-5 py-3 text-xs text-destructive font-medium">
                    {p.rejectionReason || "No reason provided"}
                  </td>

                  {/* Status */}
                  <td className="px-5 py-3">
                    <span className="inline-flex items-center rounded-md px-2 py-0.5 text-[10px] font-bold bg-destructive/10 text-destructive">
                      Rejected
                    </span>
                  </td>
